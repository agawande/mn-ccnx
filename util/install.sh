...
function mn_deps {
    ...
    sudo su -c "cat $HOME/mn-ndn/util/sysctl_addon >> /etc/sysctl.conf"
...
function of {
    ...
    patch -p1 < ~/mn-ndn/util/openflow-patches/controller.patch
...
function wireshark {
    ...
    mkdir -p ~/.wireshark
    cp ~/mn-ndn/util/colorfilters ~/.wireshark
}
...
function nox {
    ...
    git am ~/mn-ndn/util/nox-patches/*tutorial-port-nox-destiny*.patch
    if [ "$DIST" = "Ubuntu" ] && [ `expr $RELEASE '>=' 12.04` = 1 ]; then
        git am ~/mn-ndn/util/nox-patches/*nox-ubuntu12-hacks.patch
    fi
